...
def ManageCourse(request):
    object_list = Course.objects.filter(owner=request.user)
...
# function based view for the list of courses
def CourseList(request,  subject=None):
    ...
    if query:
        courses = courses.filter(Q(title__icontains=query) |
                                       Q(slug__icontains=query) | Q(overview__icontains=query)
                                       ).distinct()
    ...
    context = {'subjects': subjects, 'subject': subject,'courses': page, 'page': page,'form':form}
    return render(request,template, context)
